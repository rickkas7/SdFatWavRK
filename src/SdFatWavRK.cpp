...
SdFatWavWriter::SdFatWavWriter() :
		numChannels(1), sampleRate(16000), bitsPerSample(16) {
}

SdFatWavWriter::SdFatWavWriter(uint8_t numChannels, uint32_t sampleRate, uint8_t bitsPerSample) :
...
bool SdFatWavWriter::startFile(FatFile *file) {
	bool bResult;

	this->file = file;

	header.writeHeader(numChannels, sampleRate, bitsPerSample);

	file->rewind();
	bResult = file->truncate(0);
	if (!bResult) {
		DEBUG_NORMAL(("truncate error"));
		return false;
	}

	size_t headerSize = header.getDataOffset();

	int count = file->write(header.getBuffer(), headerSize);
	if (count != (int)headerSize) {
		DEBUG_NORMAL(("write header error"));
		return false;
	}
...
bool SdFatWavWriter::updateHeaderFromLength(FatFile *file) {

	uint32_t fileSize = file->fileSize();

	size_t headerSize = header.getDataOffset();

	DEBUG_HIGH(("fileSize=%lu headerSize=%u", fileSize, headerSize));

	header.setDataSize(fileSize - headerSize);
